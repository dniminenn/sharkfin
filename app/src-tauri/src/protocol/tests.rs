...
#[test]
fn driveall_identify_matches_the_ak029_probe() {
    let tx = driveall::identify_packet();
    assert_eq!(&tx[..8], &[0xAA, 0x10, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00]);
    let mut rx = [0u8; REPORT_LEN];
    rx[..22].copy_from_slice(&[
        0x55, 0x10, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x92, 0x45, 0x0C, 0x45,
        0x80, 0x40, 0x01, 0x00, 0x00, 0x66, 0x01,
    ]);
    let (cmd, payload) = driveall::parse_reply(&rx).unwrap();
    assert_eq!(cmd, 0x10);
    let info = driveall::DeviceInfo::parse(payload).unwrap();
    assert_eq!(info.vid, 0x0C45);
    assert_eq!(info.pid, 0x8045);
}

#[test]
fn driveall_keyboard_slot_round_trips_as_hid_usage() {
    let d = [driveall::PAGE_KEYBOARD, 0, 0x04, 0];
    let s = driveall::slot_from_driveall(d);
    assert_eq!(s, [0x00, 0, 0x04, 0]);
    assert_eq!(driveall::slot_to_driveall(s), Some(d));
    let c = driveall::slot_from_driveall([driveall::PAGE_CONSUMER, 0xE9, 0, 0]);
    assert_eq!(c, [0x03, 0, 0xE9, 0]);
    assert_eq!(
        driveall::slot_to_driveall(c),
        Some([driveall::PAGE_CONSUMER, 0xE9, 0, 0])
    );
    // A modifier reported as the HID bitmask, not as its usage.
    assert_eq!(
        driveall::slot_from_driveall([driveall::PAGE_KEYBOARD, 0x02, 0, 0]),
        [0x00, 0, 225, 0]
    );
}

/// Driveall mouse buttons are a bitmask; sharkfin numbers them 240 up.
#[test]
fn driveall_mouse_buttons_translate_both_ways() {
    for (code, mask) in [(240u8, 1u8), (241, 2), (242, 4), (243, 8), (244, 16)] {
        let d = [driveall::PAGE_MOUSE, 1, mask, 0];
        assert_eq!(driveall::slot_from_driveall(d), [0x01, 0, code, 0]);
        assert_eq!(driveall::slot_to_driveall([0x01, 0, code, 0]), Some(d));
    }
    let wheel = [driveall::PAGE_MOUSE, 3, 255, 0];
    assert_eq!(driveall::slot_from_driveall(wheel), [0x01, 0, 245, 255]);
    assert_eq!(driveall::slot_to_driveall([0x01, 0, 245, 255]), Some(wheel));
}

/// The page numbers collide with ROYUAN's and mean other things, so an
/// assignment with no driveall encoding is refused, never passed through.
#[test]
fn driveall_refuses_an_assignment_it_cannot_encode() {
    for s in [
        [10, 1, 0, 0],    // Fn: driveall page 10 is TGL
        [13, 0, 0, 0],    // Light: driveall page 13 is FUNC
        [4, 1, 0, 0],     // wheel: driveall page 4 is SYSTEM_KEY
        [1, 0, 246, 251], // mouse move: no driveall action
    ] {
        assert_eq!(driveall::slot_to_driveall(s), None, "{s:?}");
    }
}

#[test]
fn driveall_rt_scale_follows_precision_byte() {
    let mut info = driveall::DeviceInfo {
        vid: 0x0C45,
        pid: 0x8045,
        version: 0,
        current_profile: 0,
    ...
    let mut raw = vec![0u8; driveall::RT_LEN];
    raw[2] = 190;
    raw[1] = 1;
    let s = driveall::rt_from_wire(&raw, &info);
    assert!((s.keys[0].travel - 1.90).abs() < 0.001);
    assert!(s.keys[0].rapid_trigger);
    info.rt_precision = 2;
    raw[2] = 0xD0;
    raw[3] = 0x07;
    let s = driveall::rt_from_wire(&raw, &info);
    assert!((s.keys[0].travel - 2.0).abs() < 0.001);
}

/// Byte 0 selects the fitted magnetic switch model. Writing sharkfin's
/// kind over it would change the board's calibration curve.
#[test]
fn driveall_rt_patch_leaves_the_axis_type_alone() {
    let info = driveall::DeviceInfo {
        vid: 0x0C45,
        pid: 0x8045,
        version: 0,
        current_profile: 0,
    ...
    let mut raw = vec![0u8; driveall::RT_LEN];
    raw[0] = 3;
    raw[1] = 0b10; // rampage, set by the board
    let key = hall::KeySwitch {
        slot: 0,
        kind: hall::KIND_SNAP,
        rapid_trigger: true,
        travel: 1.5,
    ...
    driveall::rt_patch(&mut raw, &[key], &info);
    assert_eq!(raw[0], 3, "axis type overwritten");
    assert_eq!(raw[1], 0b11, "rampage bit lost");
    assert_eq!(u16::from_le_bytes([raw[2], raw[3]]), 150);
    assert_eq!(u16::from_le_bytes([raw[4], raw[5]]), 20);
    assert_eq!(u16::from_le_bytes([raw[6], raw[7]]), 30);
    // Read back as a plain key: the block holds no kind.
    assert_eq!(
        driveall::rt_from_wire(&raw, &info).keys[0].kind,
        hall::KIND_NORMAL
    );
}

#[test]
fn driveall_led_check_bytes_are_aa55() {
    let p = LedParam {
        mode: 1,
        speed: 3,
        brightness: 4,
    ...
    let w = driveall::led_to_wire(&p, None);
    assert_eq!(&w[14..16], &[0xAA, 0x55]);
    assert_eq!(w[4], 0xFF);
    let back = driveall::led_from_wire(&w).unwrap();
    assert_eq!(back.mode, 1);
    assert_eq!(back.r, 255);
    assert_eq!(back.option, 2);
    // Secondary colour and effect sub-mode are the board's, not ours.
    let mut prev = [0u8; driveall::LED_LEN];
    prev[5..8].copy_from_slice(&[9, 8, 7]);
    prev[12] = 3;
    let kept = driveall::led_to_wire(&p, Some(&prev));
    assert_eq!(&kept[5..8], &[9, 8, 7]);
    assert_eq!(kept[12], 3);
}

#[test]
fn driveall_set_chunks_flag_the_last_packet() {
    let data = vec![7u8; 60];
    let pkts = driveall::set_packets(driveall::SET_KEY, 0, &data, true);
    assert_eq!(pkts.len(), 2);
    assert_eq!(pkts[0][0], 0xAA);
    assert_eq!(pkts[0][1], 34);
    assert_eq!(pkts[0][6], 0);
    assert_eq!(pkts[1][6], 1);
    assert_eq!(pkts[1][2], 4);
}

/// The index offsets are absolute in the same space as the index itself,
/// so the first block starts at 400 and each one after it follows on.
#[test]
fn driveall_macro_relayout_packs_after_the_index() {
    let blocks = vec![
        Some(vec![2, 0, 0, 0, 1, 2, 3, 4]),
        None,
        Some(vec![4, 0, 0, 0, 5, 6, 7, 8, 9, 10, 11, 12]),
    ];
    let (index, payload) = driveall::macro_relayout(&blocks);
    assert_eq!(index.len(), driveall::MACRO_INDEX_LEN);
    assert_eq!(u32::from_le_bytes(index[0..4].try_into().unwrap()), 400);
    assert_eq!(u32::from_le_bytes(index[4..8].try_into().unwrap()), 0);
    assert_eq!(u32::from_le_bytes(index[8..12].try_into().unwrap()), 408);
    assert_eq!(payload.len(), 20);
    assert_eq!(&payload[..8], &blocks[0].as_ref().unwrap()[..]);
}

/// Both backends run this plan, so the refusal has to live in the plan and
/// not in either of them.
#[test]
fn driveall_key_plan_refuses_what_it_cannot_encode() {
    assert!(driveall::rmw_key(0, [10, 1, 0, 0], false).is_err());
    assert!(driveall::rmw_key(200, [0, 0, 4, 0], false).is_err());
    let plan = driveall::rmw_key(3, [0, 0, 0x04, 0], true).unwrap();
    assert_eq!(plan.get, driveall::GET_FN_KEY);
    assert_eq!(plan.set, driveall::SET_FN_KEY);
    assert_eq!(plan.len, driveall::KEYMAP_LEN);
    let mut raw = vec![0u8; driveall::KEYMAP_LEN];
    (plan.patch)(&mut raw).unwrap();
    assert_eq!(&raw[12..16], &[driveall::PAGE_KEYBOARD, 0, 0x04, 0]);
}

/// The plan reads before it writes, so the board's own bytes survive.
#[test]
fn driveall_led_plan_keeps_the_boards_own_bytes() {
    let p = LedParam {
        mode: 2,
        speed: 1,
        brightness: 3,
    ...
    let plan = driveall::rmw_led(&p);
    let mut raw = vec![0u8; driveall::LED_LEN];
    raw[5..8].copy_from_slice(&[1, 2, 3]);
    raw[12] = 4;
    (plan.patch)(&mut raw).unwrap();
    assert_eq!(&raw[5..8], &[1, 2, 3]);
    assert_eq!(raw[12], 4);
    assert_eq!(raw[0], 2);
    assert_eq!(&raw[1..4], &[9, 8, 7]);
}

/// A macro's mouse action carries the keymap's button mask, not a 1-based
/// index. Getting this wrong wrote middle-click where back was asked for.
#[test]
fn driveall_macro_mouse_buttons_use_the_button_mask() {
    for (button, mask) in [(0u8, 1u8), (1, 2), (2, 4), (3, 8), (4, 16)] {
        let m = Macro {
            repeat: 1,
            events: vec![MacroEvent::MouseButton {
                button,
                pressed: true,
                delay_ms: 7,
            }],
        };
        let block = driveall::macro_to_driveall(&m).expect("encodable");
        assert_eq!(block[6], mask, "button {button}");
        let back = driveall::macro_from_driveall(&block);
        assert_eq!(
            back.events, m.events,
            "button {button} did not survive the round trip"
        );
    }
}

/// A mask the board reports that sharkfin has no button for is dropped
/// rather than clamped onto a neighbouring button.
#[test]
fn driveall_macro_drops_a_mouse_action_it_cannot_name() {
    let block = [2u8, 0, 0, 0, 5, 0, 32, 0x90];
    assert!(driveall::macro_from_driveall(&block).events.is_empty());
}

/// Driveall's repeat modes are ordered 0, 2, 1 with the count bound to 1,
/// so sharkfin's gen2 mode byte is not passed through.
#[test]
fn driveall_macro_key_is_bound_with_the_vendor_default_mode() {
    for mode in 0..3u8 {
        assert_eq!(
            driveall::slot_to_driveall([0x09, mode, 12, 0]),
            Some([driveall::PAGE_MACRO, 12, 0, 0]),
            "mode {mode}"
        );
    }
    assert_eq!(
        driveall::slot_from_driveall([driveall::PAGE_MACRO, 12, 2, 1]),
        [0x09, 0, 12, 0]
    );
}

/// An erased index entry reads as `FF FF`, which unclamped asks for 128 KB
/// of paced reads per slot.
#[test]
fn driveall_macro_body_length_is_bounded_by_the_boards_own_budget() {
    assert_eq!(driveall::macro_body_len(&[0xFF, 0xFF], 2048), 2048);
    assert_eq!(driveall::macro_body_len(&[0xFF, 0xFF], 0), 512);
    assert_eq!(driveall::macro_body_len(&[8, 0], 2048), 16);
    assert_eq!(driveall::macro_body_len(&[], 2048), 0);
}

/// The vendor prints `13 01` as 1.13: packed decimal low byte, hundreds in
/// the high one. Showing the raw u16 made an owner's firmware unrecognisable.
#[test]
fn driveall_version_reads_the_way_the_vendor_prints_it() {
    let info = |lo, hi| {
        let mut payload = [0u8; 48];
        payload[8] = lo;
        payload[9] = hi;
        driveall::DeviceInfo::parse(&payload).unwrap()
    };
    assert_eq!(info(0x13, 1).version_string(), "1.13");
    assert_eq!(info(0x55, 1).version_string(), "1.55");
    assert_eq!(info(0x07, 0).version_string(), "0.07");
}
